...
UART::UART(USART_TypeDef* USART, uint32_t rcc, GPIO* TX, GPIO* RX, uint32_t TX_Peripheral, uint32_t RX_Peripheral, IRQn IRQ)
{
	this->USART = USART;
	this->rcc = rcc;
	this->TX = TX;
...
void UART::init(uint32_t BaudRate)
{
	...
	this->RX->init(GPIO_Mode_IN_FLOATING, GPIO_Speed_50MHz);
